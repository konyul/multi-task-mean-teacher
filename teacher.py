...
def main():
    ...
    def create_model(ema=False):
        print("=> creating {ema}model '{arch}'".format(
            ema='EMA ' if ema else '',
            arch=args.arch))
    ...
    for epoch in range(args.epochs):
        s = time.time()
        train(trainloader, model,ema_model, criterion, optimizer,epoch,auxiliary=args.auxiliary,augmentation=args.augmentation,start_time=start_time,log_directory=log_directory)
        scheduler.step()
        ...
        prec1 = validate(testloader, model, optimizer,criterion,global_step,auxiliary=args.auxiliary,start_time=start_time,log_directory=log_directory)
        
        print("Evaluating the EMA model:")
        ema_prec1 = validate(testloader, ema_model, optimizer,criterion,global_step,auxiliary=args.auxiliary,start_time=start_time,log_directory=log_directory)
...
def train(trainloader, model,ema_model, criterion, optimizer,epoch,auxiliary,augmentation,start_time,log_directory):
    global global_step
    ...
    for i, (input, target) in enumerate(trainloader):
        ...
        if auxiliary == 'rotation':
            ...
            ema_output,ema_output_aux = ema_model(input_var)
            output, output_aux = model(input_var)

            output = output[::num_aug]
            ema_output = ema_output[::num_aug]

            ema_output,ema_output_aux = Variable(ema_output.detach().data,requires_grad=False),Variable(ema_output_aux.detach().data,requires_grad=False)
            
            class_loss = criterion(output,target_var)
            soft2 = F.log_softmax(output,dim=1)
            ema_class_loss = torch.mean(-torch.sum(F.softmax(ema_output)*soft2,dim=1))

            soft = F.log_softmax(output_aux,dim=1)
            consistency_loss = torch.mean(-torch.sum(F.softmax(ema_output_aux)*soft,dim=1))
            auxiliary_loss = torch.mean(-torch.sum(target_aux*soft,dim=1))
            
            

            lamb = sigmoid_rampup(epoch,args.epochs)
            if args.ema_class_loss == True:

                loss = lamb*ema_class_loss + (1-lamb)*class_loss + lamb*consistency_loss + (1-lamb)*auxiliary_loss
            else:
                loss = class_loss + lamb*consistency_loss + (1-lamb)*auxiliary_loss
            
            #loss = criterion(output,target_var) + torch.mean(-torch.sum(target_aux*soft,dim=1))
            output, output_aux,ema_output,ema_output_aux= output.float(), output_aux.float(),ema_output.float(),ema_output_aux.float()

            prec1 = accuracy(output.data, target_var)[0]
            prec_aux = accuracy(output_aux.data,torch.argmax(target_aux,dim=1))[0]
            
            ema_prec1 = accuracy(ema_output.data, target_var)[0]
            prec_ema_aux = accuracy(ema_output_aux.data,torch.argmax(target_aux,dim=1))[0]
...
def validate(testloader, model,optimizer,criterion,global_step,auxiliary,start_time,log_directory):
    """
    Run evaluation
    """
    losses = AverageMeter()
